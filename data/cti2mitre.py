...
class CTI2MITREDataset(data.Dataset):
    def __init__(
        self,
        data_path,
        tokenizer_name,
        ...
        alldata = alldata.dropna(subset=['label_tec', 'sentence'])

        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)

        encoder_path = os.path.join(save_dir, "cti2mitre_label_encoder.pkl")
        ...
        if os.path.exists(encoder_path):
            ...
            encoder.fit(alldata[['label_tec']])
            with open(encoder_path, 'wb') as f:
                pickle.dump(encoder, f)

        print("Number of categories:", len(encoder.classes_))

        self.data = self._tokenize(alldata['sentence'].tolist())
        self.targets = torch.from_numpy(encoder.transform(alldata[['label_tec']])).to(torch.int64)
        ...
        if self.transform:
            ...
            self.data_aug_ins = self._tokenize(aug.augment(alldata['sentence'].tolist()))
            ...
            self.data_aug_sub = self._tokenize(aug.augment(alldata['sentence'].tolist()))
...
def create_cti2mitre_dataset(
    ...
    splits_file = os.path.join(save_dir, f"cti2mitre_splits_seed_{seed}.json")

    dataset = CTI2MITREDataset(data_path, tokenizer_name, save_dir, transform)
...
def create_cti2mitre_ood_dataset(
        data_path,
        tokenizer_name,
        num_id_classes=40,
        seed=0,
